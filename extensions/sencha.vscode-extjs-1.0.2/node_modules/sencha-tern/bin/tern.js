...
// changed by MB 9/15 - we always want this to be on so that class information stays loaded
var persistent = true; // process.argv.indexOf("--persistent") > -1;
...
var defaultConfig = {
  ...
  plugins: {
    ext: { }, // added by MB
    jasmine: process.argv.indexOf("--jasmine") !== -1 // added by MB
  },
  ecmaScript: true,
  ecmaVersion: 6,
...
var httpServer = http.createServer(function(req, resp) {
  ...
  /* begin added by MB 11/2015 so that plugins can respond to custom urls */
  var response = server.signalReturnFirst('request', target);
...
httpServer.listen(port, host, function() {
  ...
  if (!noPortFile) {
    var portFile = path.resolve(projectDir, '.sencha', 'temp', '.ide', portFileName);
  ...
  if (process.send) process.send({port: port}); // helpful when running with child_process.fork as in ext-ace (orion)
  process.on("SIGINT", function() { process.exit(); });
  process.on("SIGTERM", function() { process.exit(); });

  process.on('uncaughtException', function(e) {
    console.log('uncaught exception', e.message, reportError.getFrames(e))
  });
...
function respondSimple(resp, status, text) {
  resp.writeHead(status, responseHeader);
  resp.end(text);
  if (verbose) console.log("Response: " + status + " " + text);
}

/**
 * Sends a response that includes the error stack frames as an array so that IDE plugins can report it to rollbar
 * @param {HTTPResponse} resp
 * @param {Error} err
 * @param {HTTPRequest} request
 * @param {String} [msg] (optional)
 */
function respondError(resp, err, req, msg) {
  console.log('error', err.stack);
  reportError.report(err, req);

  msg = msg || err.message || 'Error processing request';

  try {
    var content = JSON.stringify({ message: msg, stack: reportError.getFrames(err) });
    resp.writeHead(500, responseHeader);
    resp.end(content);
  } catch(e) {
    respondSimple(resp, 500, "An unknown error occurred");
  }
}

function respond(resp, doc, req) {
  try {
