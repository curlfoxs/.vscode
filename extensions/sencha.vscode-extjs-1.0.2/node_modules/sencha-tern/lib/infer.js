...
(function(root, mod) {
  ...
  // added by MB 6/2016 to prevent memory leaks
  var forward = [];
  ...
  AVal.prototype = extend(ANull, {
    ...
    propagate: function(target, weight) {
      ...
      // begin change made by MB 6/2016 to prevent memory leaks
      // (this.forward || (this.forward = [])).push(target);
      this.forward = this.forward || [];
      if (target.origin && target.origin.indexOf('ecma') !== -1) this.forward.push(target);
      // end change
    ...
    getFunctionType: function() {
      // modified by MB 11/2015 to return an AVal if multiple function types are found
      var result = new AVal();

      for (var i = this.types.length - 1; i >= 0; --i)
        if (this.types[i] instanceof Fn) result.addType(this.types[i]);

      return result.types.length === 1 ? result.types[0] : result;
    },
    getObjType: function() {
      var seen = null;
      for (var i = this.types.length - 1; i >= 0; --i) {
    ...
    toString: function(maxDepth, parent) {
      ...
      // added by MB 12/2015 to speed up completion on large projects
      return _.uniq(_.pluck(this.types, 'name').map(function(n) { return n ? n.replace('.prototype', '') : '?'; })).join('|');

      // removed by MB 12/2015 to speed up completion on large projects
    ...
    // added by MB 11/2015 - this allows classes with multiple constructors to return values.  For example, classes like
    // Ext.panel.Panel exist in multiple toolkits.  Without this code "new Ext.panel.Panel()." would not complete members because
    // getFunctionType only returns the first function type.
    computeRet: function(self, args) {
      var retType = new AVal();

      this.types.forEach(function(t) {
        if (t.computeRet) {
          var ret = t.computeRet(self, args);
          if (ret) retType.addType(ret);
        } else if (t.retval) {
          t.retval.propagate(retType);
        }
      });
  ...
  var HasMethodCall = constraint({
    ...
    addType: function(obj, weight) {
      ...
      // added by MB - 11/2015 - fixes TypeError: Object function hasOwnProperty() { [native code] } has no method 'propagate'
      var prop = obj.getProp(this.propName);
      if (prop.propagate) prop.propagate(callee, weight);
    },
  ...
  function withWorklist(f) {
    ...
    for (var i = 0; i < list.length; i += 4) {
      ...
      // changed by MB 6/2016, tern occasionally gets here before list[i+1] is a function, not a type.  Not sure why
      // item.addType(list[i], list[i + 2]);
      var item = list[i + 1];
      if (item.addType) item.addType(list[i], list[i + 2]);
    }
    cx.workList = null;
    return ret;
  }
  ...
  exports.purge = function(origins, start, end) {
    // added by MB 10/2015 so plugins can purge indexes and remove files that should not be purged
    cx.parent.signal("purge", origins);
    ...
    // added by MB 6/2016 to fix memory leak in forward arrays in definition objects.
    for (var name in cx.definitions) {
      var def = cx.definitions[name];
      for (var prop in def) {
        var obj = def[prop];
        if (obj.purge) obj.purge(test);
      }
    }
  };

  // made case-insensitive by MB 10/2015 - windows file paths have inconsistent casing from VS
  function makePredicate(origins, start, end) {
    if (!Array.isArray(origins)) origins = [origins];
    origins = origins.map(function(o) { return o.toLowerCase() });
    if (end == null) return function(n) { return n.origin && origins.indexOf(n.origin.toLowerCase()) > -1; };
    return function(n, pos) { return pos && pos.start >= start && pos.end <= end && n.origin && origins.indexOf(n.origin.toLowerCase()) > -1; };
  }

  /*
  // Original case-sensitive version from tern
  function makePredicate(origins, start, end) {
  ...
  */
  ...
  Obj.prototype.purge = function(test) {
    ...
    // added by MB 6/2016 to fix memory leak where onNewProp array is never cleared
    for (var p in this.maybeProps) {
      var av = this.maybeProps[p];
      if (test(av, av.originNode))
        delete this.maybeProps[p]
      av.purge(test);
    }

    // added by MB 6/2016 to fix memory leak where onNewProp array is never cleared
    if (this.onNewProp) for (var i=0; i<this.onNewProp.length; i++) {
      var scope = this.onNewProp[i];
      if (test(scope))
  ...
  var typeFinder = {
    ...
    MemberExpression: function(node, scope) {
      ...
      // replaced with the following 8/2015 by Mark Brocato
      // This makes completion work on classes in namespaces that are also singletons.
      // For example new Ext.panel.Panel().<caret> would work but Ext.ProgressBar.<caret> would not.
      var propN = propName(node), obj = findType(node.object, scope);
  ...
  var scopeAt = exports.scopeAt = function(ast, pos, defaultScope) {
    ...
    // Added by MB 10/2015 - Allow plugins to provide a scope.  We use this for mvc references.
    if (cx) {
      var scope = cx.parent.signalReturnFirst('scopeAt', ast, pos);
