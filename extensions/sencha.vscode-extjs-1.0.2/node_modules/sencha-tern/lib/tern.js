...
(function(root, mod) {
  ...
  global.tern = {}; // added by MB 9/2015
  ...
  Server.prototype = signal.mixin({
    ...
    reset: function() {
      ...
      this.files = []; // added by Mark, 12/2016
  ...
  function doRequest(srv, doc, c) {
    ...
    for (var i = 0; i < files.length; ++i) {
      var file = files[i];
      if (file.type == "delete") {
        if (srv.signalReturnFirst("delFile", file.name)) {
          return;
        } else {
  ...
  function analyzeFile(srv, file) {
    infer.withContext(srv.cx, function() {
      ...
      if (srv.signalReturnFirst("beforeLoad", file) === false) return; // added by MB 11/2015 - allow plugins to prevent analysis
  ...
  function compareCompletions(a, b) {
    // changed to case-insensitive sorting by MB 11/2015
  ...
  var addCompletion = exports.addCompletion = function(query, completions, name, aval, depth) {
    ...
    if (aval && typeInfo) {
      ...
      // added by MB 10/2016 so we can show the correct icon in VSCode for classes
      maybeSet(rec, "isClass", type && type.hasOwnProperty('extJSClass'));

      // added by MB 10/2015 so we can add a sencha icon to Ext class members (including user classes)
      maybeSet(rec, "sencha", aval.sencha || type && type.sencha);
  ...
  function findCompletions(srv, query, file) {
    ...
    if (fromPlugin) {
      if (query.sort !== false) fromPlugin.completions = fromPlugin.completions.sort(compareCompletions);
      return fromPlugin;
    }
    ...
    // added by MB 10/2015 - ask plugins for custom scope - this is how we implement scopes for mvc dependencies
    var scope = infer.scopeAt(file.ast, wordStart, file.scope);
    ...
    if (objLit) {
      ...
      if (path) {
        objType = infer.def.parsePath(path, scope).getObjType();
        prop = word;
      }
    }
    ...
    if (prop != null) {
      ...
      infer.forAllLocalsAt(file.ast, wordStart, scope, gather);
      if (query.includeKeywords && scope.includeKeywords !== false /* added by MB 10/2015 - allow scope determine not to include keywords.  This allows us to leave keywords out of mvc scopes */) jsKeywords.forEach(function(kw) {
  ...
  function findTypeAt(srv, query, file) {
    ...
    /*
     * 12/2015 by MB - allow plugins to override how types are described by the "type" query.  This allows us to
     * include information about the parameter at the cursor when describing methods
     */
    ...
    return srv.signalReturnFirst('describeType', expr, describeType) || describeType(expr);
  }
  ...
  function findDocs(srv, query, file) {
    ...
    // added by MB 10/2015 - allow plugins to help with finding docs
    var type = srv.signalReturnFirst('findDef', file, query) || findExprType(srv, query, file, expr);
    ...
    result = srv.signalReturnFirst('findDocs', type, file, result) || result;
    return clean(result);
  }
  ...
  function storeTypeDocs(query, type, out) {
    ...
    if (!out.originPos) out.originPos = type.originPos; // added 9/15 by MB
  ...
  var storeSpan = exports.storeSpan = function(srv, query, span, target) {
    ...
    if (span.span) {
      ...
      // changed 10/2015 by MB so that line/char positions can be returned for uncached files
      if (m) {
        target.start = Number(m[1]);
        target.end = Number(m[4]);
      }

      if (query.lineCharPositions) {
        var file = srv.findFile(span.origin) || { text: require('fs').readFileSync(span.origin, 'utf8') };
        target.start = asLineChar(file, target.start);
        target.end = asLineChar(file, target.end);
      }
    } else {
  ...
  function findDef(srv, query, file) {
    ...
    try {
      // added by MB 10/2015 - allow plugins to do their own definition lookup
      var types = srv.signalReturnFirst('findDef', file, query) || findExprType(srv, query, file, expr);
    } catch (e) {
      // added 12/2016 by Mark
      if (e.message === 'No type found at the given position.') {
        return {};
      } else {
        throw e;
      }
